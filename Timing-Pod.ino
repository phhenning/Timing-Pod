...
unsigned long baseTimeMs;                                   // Base time in sec from RTC to withc we sync the ms count
bool battGood = 1;
...
void writeCard() {
    ...
    for ( int i =0; i < msSize; i++ ){
        msBuffer[i] = (int)((realMs >> (i*8)) & 0xFF);
    }
    for (int i =0; i < 16; i++){
      Serial.print(msBuffer[i], HEX);
    }
    Serial.println();
    ...
    status = mfrc522.MIFARE_Write(block, msBuffer, 16);
    if (status != MFRC522::STATUS_OK) {
...
void writeCardOld() {
    unsigned long realMs = getAfricaTimeMs();       // read the ms time from int counter
    String timestamp     = String(realMs);          // converts type unsigned long to type string for getByte function         
...
void updateLcdScreen()    {
    ...
    unsigned long realTimeS = realMs/1000; // convert to sec for easy display
...
void initAfricaTimer(){
    ...
    *digitalPinToPCMSK(SQ_WAVE) |= bit (digitalPinToPCMSKbit(SQ_WAVE));  // enable pin
    ...
    unsigned long baseTimeS = RTC.get();
