...
void setup() 
{
    ...
    myGLCD.InitLCD();                                     // initialise LCD amd set contrasst in brackets (90=darker, 70 default)
    initIoPins();
    initMfrc522();
    configrePod();
    initAfricaTimer();
...
void setupAndBeep(){
   if ( thisPod.role == START_POD ) {
        int length = 4;
        int melody[length] = {NOTE_A7, NOTE_A7, NOTE_AS7, NOTE_AS7};        // notes in the melody:
...
void initAfricaTimer(){
    ...
    *digitalPinToPCMSK(SQ_WAVE) |= bit (digitalPinToPCMSKbit(SQ_WAVE));  // enable pin
    ...
    // Regester our base time now with only  H:m:s; 
    baseTimeS     =  (hourToday * 60 * 60) + (minToday * 60) + secToday;   
    // Regester our base time now with  D H:m:s; 
